...
class AnalysisWorkspace:
    ...
    def complete(self, stage: str, artifacts: Iterable[Path] = ()) -> None:
        value = self._stage(stage)
        value.status = "completed"
        value.artifacts = [str(Path(path)) for path in artifacts]
        value.error = ""
        self.save()
    ...
    def can_reuse(self, stage: str, request: Optional[Mapping[str, object]] = None) -> bool:
        value = self._stage(stage)
        if value.status != "completed":
            return False
        if request is not None and self.state.request_fingerprint != request_fingerprint(request):
            return False
        return bool(value.artifacts) and all(Path(item).exists() for item in value.artifacts)
