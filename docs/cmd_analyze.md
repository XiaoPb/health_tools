...
| `--activity` | `auto`、`rest`、`walk`、`run`、`cycle`、`strength`、`interval`、`recovery`、`other`；显式值优先于路径和运动特征 |
...
PPT 正文按模板现有槽位填写活动、文件名、精简 check 结果、诊断正文、主图和副图。主图使用 PSD/已有时频图，副图为本次报告重新生成的时间片段图；图片等比居中，不修改模板中的 Logo、页脚、字号或槽位边界。每条数据只保留一页正文，算法原因页不会显示建议行。

本报告属于工程数据诊断，不诊断血管疾病、心律失常或临床状态。心律不齐只可表述为参考数据可能受影响并建议人工复核。
