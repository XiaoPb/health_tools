...
报告只引用支撑当前原因的关键图。每组数据记录最多生成一页明细 PPT，不因 Polar warning 或其他附加证据额外分页；同一记录的主图、副图和检查/诊断文字合并在该页。普通正常文件不额外生成图；异常文件、`--focus` 文件和已有 PSD 证据的“证据不足”文件才进入证据页。帧或时间戳异常无法由 `plot` 表达时，才单独生成采样间隔图。
