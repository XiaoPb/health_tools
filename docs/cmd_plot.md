...
| `--time-range` | 按秒指定普通绘图范围，格式 `START-END`；短于图类型最小宽度时自动扩展 |
| `--fft-start` | FFT 起始时间（秒），必须与 `--fft-duration` 同时指定 |
| `--fft-duration` | FFT 使用长度（秒），严格按指定长度绘制，必须与 `--fft-start` 同时指定 |
...
1. 原始 ACCX、ACCY、ACCZ 三轴，以及独立 Y 轴的合加速度 `ACCRMS=sqrt(ACCX²+ACCY²+ACCZ²)`；
2. 去基线并经过 `--bandpass` 带通滤波后的 PPG；Y 轴保持对称，并按有效数据自动留出上下各 10%；
...
第一个 ACC 子图仍叠加 ACCX、ACCY、ACCZ，但三轴和 ACCRMS 分别使用独立的 Y 轴刻度。

所有普通绘图的顶部显示输入文件名。连续数据轴按有效数据自动设置范围，并在上下各保留约 10% 留白；常量数据使用安全范围，对数频谱使用正数安全范围。`--time-range` 使用左闭右开秒级范围；未指定时使用全部数据，短范围会按图类型扩展到可见的最小宽度。`--height` 控制整张图片高度，适合嵌入 PPT，不改变输出文件名或 DPI。

自动绘图会跳过全零、全 NaN、全 Inf 和非数值列；`time` 模式未指定 `--channels` 时仅默认绘制输入 CSV 中有效的 ACC 与 `rawdata*` 列。其他列必须通过 `--channels` 显式指定；显式指定的无效列会跳过，若没有任何有效列则报错。
...
指定 `--type fft` 时，每个 PPG 通道生成一张 `<输入名>_fft_<通道>.<格式>`。图片包含原始
PPG 时域波形，以及原始/带通后 PPG 的单边幅值谱；两条频谱曲线使用独立 Y 轴。横轴不包含
`0 Hz` 的 DC 点，显示范围延伸到 Nyquist 频率，并在 Raw FFT 曲线上标注幅值最高的前 3 个
非直流频率波峰（不足 3 个时按实际数量标注）。

`--fft-start START --fft-duration SECONDS` 用于精确选择 FFT 数据窗口，输出时域波形和频谱
都只使用该窗口；窗口超出数据范围或长度不足时会报错。未指定时使用整段数据；已有
`--time-range START-END` 仍可作为 FFT 的兼容时间窗口。
...
- 普通绘图输入为目录时递归查找文件，并按 CSV 并行绘图；PSD 模式按每个 VSHB 及其配套
  PSD 文件组成的 PSD 文件组并行绘图。两种模式都由 `--workers` 控制，默认和最大值均为
  8；`--workers 1` 可按稳定输入顺序串行执行。
- 普通绘图的每个 CSV 使用独立 plotter，PSD 的每个文件组也独立渲染。开始并行写入前会
  预检所有目标路径；不同输入会生成同一个目标名时立即报告“输出文件冲突”，不会覆盖或
  并发写入同一文件。
- 输入为目录时使用进度条显示进度，最终结果仍按输入顺序汇总，不受线程完成顺序影响。
