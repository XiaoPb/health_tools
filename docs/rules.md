...
`marks` 中 `id` 和 `category` 都必须唯一且为安全的单段目录名。每项支持单条件简写或组合条件两种写法。

| 字段 | 类型 | 说明 |
|---|---|---|
| `marks[].match` | string | 组合模式：`all`（全部条件同时满足，默认）或 `any`（任一条件满足） |
| `marks[].conditions` | list | 条件列表；每项含 `left`、`operator`、`threshold`（可选 `right`）。提供 `conditions` 时不能再提供单条件的 `left/operator/threshold/right`；两者都不提供则校验失败。单条件 mark 可继续使用单条件简写（等价于 `match: all` + 单个条件） |

单条件简写直接提供 `left`、`operator`、`right`、`threshold` 声明单个条件：路径格式为
...
匹配按声明顺序取首个命中；组合 mark 在精简总表中展示全部可计算条件数值（四舍五入到两位小数后逗号连接），
全部不可计算时留空；单条件 mark 展示单个数值。
...
配置 `classify` 后，该源相对路径只用于提取变量，不参与输出布局，转换结果直接写入
`{输出目录}/{类别路径}/{输出文件名}`。分类条件未命中时直接写入输出根目录下的 `{default}`
目录，保证转换产物不丢失。目录批处理中分类输出若发生同名冲突，会追加递增序号避免覆盖；
单文件转换继续沿用原目标文件名。`rename` 生成新的输出文件名（split 分段时追加 `_{序号}`）。
`split` 与 `classify` 可同时配置：先分割并逐段转换，再对每段独立分类。
若内嵌 `classify` 配置了 `filters.min_rows` 或 `filters.min_size_kb`，过滤也在逐段转换后、
分类写出前执行：行数按转换后的 DataFrame 计算，大小按输出 CSV 配置序列化后的字节数计算。
不满足任一阈值的分段不会生成文件；保留的分段沿用原始分段序号，全部分段均不满足时该源
文件记为跳过。
