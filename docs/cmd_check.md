...
- 普通检查模式使用进度条显示并行处理进度。文件任务采用有界线程调度，在途任务窗口最多为 `workers * 2`，不会一次性为目录中的全部文件创建 Future；任务完成后按输入顺序汇总报告。
- `--workers` 控制文件级并行度。增大线程数不保证线性提速，尤其是 ACC 循环检测等 CPU 密集型检查；取消操作会停止提交新文件并取消尚未开始的任务。
...
仓库提供可重复运行的合成数据基准，不会写入生产测试夹具：

```bash
python tests/benchmarks/bench_check_performance.py --files 100,500,1000 --workers 1,2,4,8
```

输出包含每组配置的耗时、可用时的进程峰值 RSS，以及 CSV 读取、时间戳解析、采样位置构建、
秒采样和 ACC 输入准备次数。不同机器、磁盘与检查项的瓶颈不同，应比较同一环境中的结果，
不应把某个固定耗时或线程提速比例作为正确性判断。
