...
外部算法使用参数列表直接启动，不经过 Windows Shell。因此，路径中的 `&` 不会被解释为
命令分隔符，中文、空格和特殊字符会作为完整的 argv 参数原子传递。程序只在支持文件计数
和结果序号映射时，排除系统代码页（`mbcs`）无法编码的 CSV；不会因此从输入目录移动或
屏蔽这些文件。外部老旧 EXE 能否自行枚举和处理这些路径仍取决于其自身能力，需用实际版本
验证。
...
默认终端显示聚合进度和任务状态，并在摘要后额外列出每个子进程日志路径；`-v/--verbose`
改为显示逐任务诊断，包括返回码、计数、错误等，不重复列出日志清单。两种模式都不显示
外部命令、`log_tail`、stdout 或 stderr；完整子进程输出应到上述日志中查看。
...
前三行。CSV 表头展开后的列数和顺序必须与 `rules/chip/<chip>.yaml` 展开后的列定义一致；
除完整 `REF_RESULT0`～`REF_RESULT15` 对应位置外，其他列名必须完全一致。完整 REF 对应的
这 16 个位置允许使用任意非空列名；仅允许首列表头带 UTF-8 BOM，不会忽略大小写或额外空格。

编码错误、文件行数不足、表头无法解析或表头不匹配的文件会移动到输入目录同级的
...
当输入目录包含一级子目录时，`offline` 会把每个可处理子目录作为独立任务运行。任务目录名保留
Unicode 字母和数字；空格、`&`、括号等特殊字符会压缩为下划线，任务编号仍会作为前缀避免重名。
因此包含中文和 `&` 的输入路径可以作为完整参数传递给离线工具，不会被 Windows Shell 拆分。

每个版本输出目录都会保存任务日志：

```text
<output>/<version>/offline_logs/<任务ID>.log
```

日志以 UTF-8 记录输入目录、输出目录、实际命令以及外部算法的 stdout/stderr。失败后重试会追加到
同一个日志文件，并通过尝试次数分隔。命令行默认只显示进度和任务结果摘要，不逐行显示外部算法输出；
失败摘要会给出对应日志路径。成功合并任务结果后会清理 `.offline_tasks/`，但会保留 `offline_logs/`；
新一轮同版本跑库开始前会清理该版本已有的日志和其他可再生产物。
